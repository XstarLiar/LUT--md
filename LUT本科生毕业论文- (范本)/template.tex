...
\title{{考古报告智能信息提取系统的}{数据防篡改机制设计与实现}}
...
\entitle{{Design and implementation of data tamper-proof mechanism of }{intelligent information extraction system for archaeological report}}
...
\mysignature{
    % \raisebox{-5pt}{
    \includegraphics[width=40pt]{姓名.png}
    % }
}
...
\ZhAbstract{本题目旨在研究考古报告智能信息提取过程中的数据防篡改机制。针对考古报告从PDF文档输入到最终知识提取的全流程，设计并实现一套安全防护方案。主要研究内容包括：原始PDF文档的完整性保护机制、OC文本提取过程的安全审计、多模态数据关联的一致性校验，以及基于区块链的全流程数据存证机制。通过数字签名、文件指纹区块链等技术，确保数据处理过程的可信度和可追溯性。

    研究将形成一套完整的技术方案和系统原型，保障考古数据提取结果的真实性和完整性。本题目的创新点在于将区块链技术应用于考古数据处理过程的存证，设计了多模态数据处理的完整性校验方法，并提出了适合考古领域的数据防篡改机制。}
    {MetaGPT，数字签名，信息安全}%这一行是关键词


%英文摘要
\EnAbstract{This project focuses on anti-tampering mechanisms for archaeological report intelligent information extraction. It aims to design and implement a security protection plan for the entire process from PDF document input to final knowledge extraction. The main research content includes: integrity protection mechanism for original PDF documents, security audit of OCR text extraction process, consistency verification of multi-modal data association, and blockchain-based full-process data notarization mechanism. By using technologies such as digital signature and file fingerprint blockchain, it ensures the trustworthiness and traceability of the data processing process.
The research will form a complete technical solution and system prototype, ensuring the authenticity and integrity of the results of archaeological data extraction. The innovation of this topic lies in the application of blockchain technology to the notarization of archaeological data processing, the design of integrity verification methods for multi-modal data processing, and the proposal of anti-tampering mechanisms suitable for the field of archaeology.\fontspec{Times New Roman} {abcdQR}}
